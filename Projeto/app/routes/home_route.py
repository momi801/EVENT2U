...
def telaInicial():
    ...
    CATEGORY_MAP = {
        ...
        # Shows, Festivais & Festas (música)
        'shows': ['musica'], 
        # Espetáculos, Teatro, Comédia (arte & cultura)
        'theater': ['arte & cultura'],
        # Esportes (esportes)
        'sports': ['esportes'], 
        # Negócios, Educação & Workshop (negócios, educação, tecnologia)
        'business': ['negocios', 'educacao', 'tecnologia'],
        # Outros (outro)
        'cultural': ['arte & cultura'], # Seu HTML original tem 'cultural' para 'Lazer, passeios'. Se quiser agrupar com arte/cultura, deixe assim.
        'other': ['outro'] # Adicionei 'other' se o JS usar o data-category 'other'
    }
    ...
    # 2. CALCULAR MÉTRICAS (Vendas e Preço Mais Baixo)
    ...
    if email_sessao:
        ...
        if usuario_existente is None:
            # A sessão contém um email, mas o usuário não existe no banco (deletado ou banco resetado).
            # Limpa a sessão quebrada e assume que não há login.
    ...
    for evento, vendas_count, valor_min in eventos_com_dados:
        ...
        e = {
            ...
            # 'data': evento.data, # REMOVIDO: O campo 'data' não existe na model
    ...
    # 5. LÓGICAS DE DESTAQUE E ORDENAÇÃO
    # O restante do código de ordenação (eventos_caros, evento_famoso, eventos_recentes)
    # está perfeitamente correto, pois usa os novos campos calculados do dicionário.
