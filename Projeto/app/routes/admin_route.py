...
def menuOrganizador():
    ...
    # 1. Calcula a taxa de conversão (participantes / ingressos) para cada evento.
    ...
    # 2. Verifica se há eventos com ingressos para evitar divisão por zero novamente
        num_eventos_com_ingressos = len(taxas_de_conversao)
    
        if num_eventos_com_ingressos > 0:
            # 3. Soma as taxas e divide pelo número de eventos que foram considerados.
    ...
    # Se for GET, ou após o POST, renderiza o template passando o objeto 'organizador'
...
def perfilOrganizador():
    ...
    if request.method == 'POST':
        ...
        novo_telefone = request.get('telefone')
    ...
    # 5. RENDERIZAÇÃO
    # Passa o objeto 'organizador' (com dados atualizados ou originais) para o template
